...
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
  ...
  /*  import { createPopper, right} from '@popperjs/core';
   createPopper('.tooltip__btn', tooltip, {
     placement: 'right'
   }); */
  ...
  const apartments = new swiper__WEBPACK_IMPORTED_MODULE_1__["default"]('.apartments-swiper', {
    slidesPerView: 4,
    loop: true,
    centeredSlides: true,
    navigation: {
  ...
  document.querySelectorAll('.feedback').forEach(el => el.addEventListener('click', function (e) {
    const modalPol = document.getElementById('modal-politic');
    modalPol.classList.remove('graph-modal-open', 'fade', 'animate-open');
    new GraphModal().open('feedback');
  }));
  document.querySelectorAll('.politic').forEach(el => el.addEventListener('click', function (e) {
    const modalForm = document.getElementById('modal-form');
    modalForm.classList.remove('graph-modal-open');
    modalForm.classList.remove('fade');
    modalForm.classList.remove('animate-open');
    new GraphModal().open('politic');
  }));
  ...
  if (bodyWidth < 768) {
    const onScrollHeader = () => {
      ...
      window.addEventListener('scroll', () => {
        ...
        if (currentScroll > prevScroll + 40 && !headerHidden()) {
